...
public class Step02_01_TaskA_GenVSM {
...
//		String[] tokensStr  = tempLine.split("\t");
	    ...
	    for (int topic_idx = 0; topic_idx < topic2filesuffix.size(); topic_idx++) {
			String wordMapStr = resultsPathStr + "usrWordDict_" + topic2filesuffix.get(topic_idx);
			
			String vsmBW_train_labeledStr = resultsPathStr + "/vsm_train_labeled_"
											+ topic2filesuffix.get(topic_idx);
			String vsmBW_train_unlabeledStr = resultsPathStr+"/vsm_train_unlabeled_"
											  + topic2filesuffix.get(topic_idx);
			String vsmBW_test_unlabeledStr = resultsPathStr+"/vsm_test_unlabeled_"
					  						 + topic2filesuffix.get(topic_idx);
